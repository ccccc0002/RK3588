...
def create_fastapi_app(runtime: P0Runtime | None = None, bootstrap_token: str = "") -> Any:
    ...
    def list_gray_rollout_batch_cache_ep(
        ...
        denied = _authorize_request(
            authorization,
            required_get_action("/api/v1/gray-rollout/plan/batch/cache") or "device:read",
        )
        if denied is not None:
            return denied
        try:
            return ok_payload(
                rt.list_gray_rollout_batch_plan_cache({"limit": limit, "include_events": include_events})
            )
        except ValueError as exc:
            return JSONResponse(status_code=400, content=error_payload("bad_request", str(exc)))
    ...
    def list_gray_rollout_batch_cache_ops_ep(
        ...
        denied = _authorize_request(
            authorization,
            required_get_action("/api/v1/gray-rollout/plan/batch/cache/ops") or "device:write",
        )
        if denied is not None:
            return denied
        try:
    ...
    @app.post("/api/v1/gray-rollout/plan/batch")
    def plan_gray_rollout_batch_ep(
        payload: dict = Body(default_factory=dict),
        authorization: str = Header(default="", alias="Authorization"),
    ):
        denied = _authorize_request(
            authorization,
            required_post_action("/api/v1/gray-rollout/plan/batch") or "device:read",
        )
        if denied is not None:
            return denied
        try:
    ...
    @app.post("/api/v1/gray-rollout/plan/batch/cache/clear")
    def clear_gray_rollout_batch_cache_ep(
        payload: dict = Body(default_factory=dict),
        authorization: str = Header(default="", alias="Authorization"),
    ):
        denied = _authorize_request(
            authorization,
            required_post_action("/api/v1/gray-rollout/plan/batch/cache/clear") or "device:write",
        )
        if denied is not None:
            return denied
        try:
            return ok_payload(rt.clear_gray_rollout_batch_plan_cache(dict(payload)))
        except ValueError as exc:
            return JSONResponse(status_code=400, content=error_payload("bad_request", str(exc)))
