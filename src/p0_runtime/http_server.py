...
_DASHBOARD_ROOT = Path(__file__).resolve().parents[2] / "apps" / "dashboard"
_DASHBOARD_ROUTE_MAP = {
    "/": _DASHBOARD_ROOT / "index.html",
    "/assets/dashboard.css": _DASHBOARD_ROOT / "dashboard.css",
    "/assets/dashboard.js": _DASHBOARD_ROOT / "dashboard.js",
}
...
class _RuntimeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        static_path = _DASHBOARD_ROUTE_MAP.get(parsed.path)
        if static_path is not None:
            if static_path.is_file():
                _file_response(self, static_path)
            else:
                _json_response(self, 404, error_payload("not_found", "dashboard asset not found"))
            return
    ...
    def do_POST(self) -> None:
        try:
            ...
            if parsed.path == "/api/v1/edge-agents/offline-jobs/lease/release":
                res = self.runtime.release_offline_job_lease(dict(body), now=_parse_time(body.get("now")))
                _json_response(self, 200, ok_payload(res))
                return

            if parsed.path == "/api/v1/edge-agents/offline-jobs/lease/complete":
                res = self.runtime.complete_offline_job_with_lease(dict(body), now=_parse_time(body.get("now")))
                _json_response(self, 200, ok_payload(res))
                return
            ...
            if parsed.path == "/api/v1/gray-rollout/plan":
                res = self.runtime.plan_gray_rollout_dependencies(dict(body))
                _json_response(self, 200, ok_payload(res))
                return
