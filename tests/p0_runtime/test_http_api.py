...
class P0HttpApiTests(unittest.TestCase):
    ...
    def _assert_window_time_gap_metrics(self, payload: dict) -> None:
        ...
        expected_missing_at_ratio = (
            round(float(expected_missing_at_count) / float(len(items)), 6) if items else 0.0
        )
        expected_invalid_at_ratio = (
            round(float(expected_invalid_at_count) / float(len(items)), 6) if items else 0.0
        )
        ...
        self.assertAlmostEqual(expected_missing_at_ratio, float(data["window_time_missing_at_ratio"]), places=6)
        self.assertAlmostEqual(expected_invalid_at_ratio, float(data["window_time_invalid_at_ratio"]), places=6)
        ...
        gaps = [
            abs(
                (
                    datetime.fromisoformat(str(items[i]["at"]))
                    - datetime.fromisoformat(str(items[i + 1]["at"]))
                ).total_seconds()
            )
        ...
        expected_range = round(max(0.0, (max(gaps) - min(gaps))), 6) if gaps else 0.0
        expected_count = len(gaps)
        expected_zero_count = sum(1 for gap in gaps if gap == 0.0)
        expected_nonzero_count = max(0, expected_count - expected_zero_count)
        expected_zero_ratio = (
            round(float(expected_zero_count) / float(len(gaps)), 6) if gaps else 0.0
        )
        expected_nonzero_ratio = (
            round(float(expected_nonzero_count) / float(len(gaps)), 6) if gaps else 0.0
        )
        if gaps:
            sorted_gaps = sorted(gaps)
            mid = len(sorted_gaps) // 2
            if len(sorted_gaps) % 2 == 1:
                median_raw = float(sorted_gaps[mid])
            else:
                median_raw = float(sorted_gaps[mid - 1] + sorted_gaps[mid]) / 2.0
            expected_median = round(median_raw, 6)
            ...
            expected_outlier_fence_span_seconds = round(
                max(0.0, float(expected_outlier_upper) - float(expected_outlier_lower)),
                6,
            )
            expected_outlier_fence_midpoint_seconds = round(
                (float(expected_outlier_upper) + float(expected_outlier_lower)) / 2.0,
                6,
            )
            expected_outlier_effective_lower_seconds = round(
                max(0.0, float(expected_outlier_lower)),
                6,
            )
            expected_outlier_effective_span_seconds = round(
                max(0.0, float(expected_outlier_upper) - max(0.0, float(expected_outlier_lower))),
                6,
            )
            expected_outlier_effective_midpoint_seconds = round(
            ...
            expected_outlier_lower_truncation_seconds = round(
                max(0.0, -float(expected_outlier_lower)),
                6,
            )
            if expected_outlier_fence_span_seconds > 0.0:
                expected_outlier_lower_truncation_ratio = round(
                    float(expected_outlier_lower_truncation_seconds)
                    / float(expected_outlier_fence_span_seconds),
                    6,
                )
            else:
            ...
            expected_outlier_lower_unclipped = not expected_outlier_lower_clipped
            expected_outlier_lower_unclipped_ratio = 0.0 if expected_outlier_lower_clipped else 1.0
            expected_outlier_lower_clip_state = "clipped" if expected_outlier_lower_clipped else "unclipped"
            expected_outlier_lower_clip_state_code = 1 if expected_outlier_lower_clipped else 0
            ...
            expected_inlier_minus_outlier_count = int(expected_inlier_count) - int(expected_outlier_count)
            expected_outlier_minus_inlier_count = int(expected_outlier_count) - int(expected_inlier_count)
            expected_outlier_inlier_count_ratio = round(
            ...
            expected_outlier_inlier_ratio_delta = round(
            ...
            expected_outlier_inlier_ratio_delta_per_mille = round(
                float(expected_outlier_inlier_ratio_delta) * 1000.0,
                6,
            )
            ...
            expected_outlier_inlier_ratio_delta_positive = expected_outlier_inlier_ratio_delta > 0.0
            ...
            expected_p99 = round(float(sorted_gaps[p99_index]), 6)
        else:
            expected_median = 0.0
            expected_p90 = 0.0
            expected_p75 = 0.0
            ...
            expected_outlier_fence_span_seconds = 0.0
            expected_outlier_fence_midpoint_seconds = 0.0
            ...
            expected_outlier_effective_coverage_ratio = 0.0
            expected_outlier_lower_truncation_seconds = 0.0
            expected_outlier_lower_truncation_ratio = 0.0
            expected_outlier_lower_clipped = False
            expected_outlier_lower_clipped_ratio = 0.0
            expected_outlier_lower_unclipped = True
            expected_outlier_lower_unclipped_ratio = 1.0
            expected_outlier_lower_clip_state = "unclipped"
            expected_outlier_lower_clip_state_code = 0
            expected_outlier_ratio = 0.0
            expected_inlier_count = 0
            expected_inlier_ratio = 0.0
            expected_inlier_minus_outlier_count = 0
            expected_outlier_minus_inlier_count = 0
            ...
            expected_outlier_inlier_ratio_delta_per_mille = 0.0
        ...
        self.assertEqual(expected_nonzero_count, int(data["window_time_gap_nonzero_count"]))
        ...
        self.assertAlmostEqual(expected_outlier_fence_span_seconds, float(data["window_time_gap_outlier_fence_span_seconds"]), places=6)
        self.assertAlmostEqual(expected_outlier_fence_midpoint_seconds, float(data["window_time_gap_outlier_fence_midpoint_seconds"]), places=6)
        ...
        self.assertAlmostEqual(expected_outlier_lower_truncation_seconds, float(data["window_time_gap_outlier_lower_truncation_seconds"]), places=6)
        ...
        self.assertEqual(expected_outlier_lower_unclipped, bool(data["window_time_gap_outlier_lower_unclipped"]))
        ...
        self.assertEqual(expected_outlier_lower_clip_state, data["window_time_gap_outlier_lower_clip_state"])
        self.assertEqual(expected_outlier_lower_clip_state_code, int(data["window_time_gap_outlier_lower_clip_state_code"]))
        ...
        self.assertEqual(expected_inlier_minus_outlier_count, int(data["window_time_gap_inlier_minus_outlier_count"]))
        self.assertEqual(expected_outlier_minus_inlier_count, int(data["window_time_gap_outlier_minus_inlier_count"]))
        ...
        self.assertAlmostEqual(expected_outlier_inlier_ratio_delta_per_mille, float(data["window_time_gap_outlier_inlier_ratio_delta_per_mille"]), places=6)
    ...
    def test_audit_recent_endpoint_requires_operator_role(self) -> None:
        ...
        self.assertAlmostEqual(
            float(op_payload["data"]["returned_items"]) / float(op_payload["data"]["window_span"]),
        ...
        self.assertEqual(
            max(
                ...
                int(
                    (
                        datetime.fromisoformat(str(op_payload["data"]["items"][0]["at"]))
                        - datetime.fromisoformat(str(op_payload["data"]["items"][-1]["at"]))
                    ).total_seconds()
                ),
            ),
        ...
        self.assertEqual(
            ...
            if len(op_payload["data"]["items"]) > 1
            else 0,
            int(op_payload["data"]["window_time_gap_max_seconds"]),
        )
        self._assert_window_time_gap_metrics(op_payload)
        ...
        if op_payload["data"]["has_more"]:
            ...
            self.assertIsNotNone(op_payload["data"]["next_query"])
            self.assertIsNotNone(op_payload["data"]["next_query_string"])
            self.assertIn(
                f"before_id={op_payload['data']['next_before_id']}",
                op_payload["data"]["next_query_string"],
            )
            self.assertEqual(op_payload["data"]["next_before_id"], op_payload["data"]["next_query"]["before_id"])
        else:
            self.assertIsNone(op_payload["data"]["next_before_id"])
            self.assertIsNone(op_payload["data"]["next_query"])
            self.assertIsNone(op_payload["data"]["next_query_string"])
        ...
        self.assertAlmostEqual(
            float(win_payload["data"]["returned_items"]) / float(win_payload["data"]["window_span"]),
        ...
        self.assertEqual(
            max(
                ...
                int(
                    (
                        datetime.fromisoformat(str(win_payload["data"]["items"][0]["at"]))
                        - datetime.fromisoformat(str(win_payload["data"]["items"][-1]["at"]))
                    ).total_seconds()
                ),
            ),
        ...
        self.assertEqual(
            ...
            if len(win_payload["data"]["items"]) > 1
            else 0,
            int(win_payload["data"]["window_time_gap_max_seconds"]),
        )
        self._assert_window_time_gap_metrics(win_payload)
        ...
        self.assertIsNotNone(win_payload["data"]["next_query"])
        self.assertIsNotNone(win_payload["data"]["next_query_string"])
        self.assertEqual(win_payload["data"]["next_before_id"], win_payload["data"]["next_query"]["before_id"])
        self.assertFalse(win_payload["data"]["next_query"]["include_total"])
        self.assertIn("include_total=false", win_payload["data"]["next_query_string"])
        ...
        self.assertAlmostEqual(
            float(total_payload["data"]["returned_items"]) / float(total_payload["data"]["window_span"]),
        ...
        self.assertEqual(
            max(
                ...
                int(
                    (
                        datetime.fromisoformat(str(total_payload["data"]["items"][0]["at"]))
                        - datetime.fromisoformat(str(total_payload["data"]["items"][-1]["at"]))
                    ).total_seconds()
                ),
            ),
        ...
        self.assertEqual(
            ...
            if len(total_payload["data"]["items"]) > 1
            else 0,
            int(total_payload["data"]["window_time_gap_max_seconds"]),
        )
        self._assert_window_time_gap_metrics(total_payload)
        self.assertEqual(
            int(total_payload["data"]["total_candidates"]) - int(total_payload["data"]["returned_items"]),
            int(total_payload["data"]["remaining_candidates"]),
        )
        self.assertIsNotNone(total_payload["data"]["next_query"])
        self.assertIsNotNone(total_payload["data"]["next_query_string"])
        self.assertTrue(total_payload["data"]["next_query"]["include_total"])
        self.assertIn("include_total=true", total_payload["data"]["next_query_string"])
