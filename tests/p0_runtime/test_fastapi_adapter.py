...
class FastApiAdapterTests(unittest.TestCase):
    def test_create_fastapi_app_behavior(self) -> None:
        ...
        if available:
            ...
            self.assertIn("/api/v1/edge-agents", paths)
            self.assertIn("/api/v1/edge-agents/register", paths)
            self.assertIn("/api/v1/edge-agents/heartbeat", paths)
            self.assertIn("/api/v1/edge-agents/offline-jobs/lease", paths)
            ...
            self.assertIn("/api/v1/offline-sync/streams", paths)
            self.assertIn("/api/v1/offline-sync/streams/upsert", paths)
