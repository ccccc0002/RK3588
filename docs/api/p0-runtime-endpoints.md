...
- `GET /api/v1/audit/recent?limit=20`
  ...
  - 200 envelope:
    ...
    - `returned_items`, `window_max_id`, `window_min_id`, `window_span`, `dense_window`, `id_gap_count`, `window_density`, `window_newest_at`, `window_oldest_at`, `window_time_parseable`, `window_time_unparseable_count`, `window_time_parseable_count`, `window_time_missing_at_count`, `window_time_invalid_at_count`, `window_time_missing_at_ratio`, `window_time_invalid_at_ratio`, `window_time_unparseable_ratio`, `window_time_parseable_ratio`, `window_time_span_seconds`, `window_time_desc_order`, `window_time_gap_max_seconds`, `window_time_gap_min_seconds`, `window_time_gap_range_seconds`, `window_time_gap_count`, `window_time_gap_zero_count`, `window_time_gap_nonzero_count`, `window_time_gap_zero_ratio`, `window_time_gap_nonzero_ratio`, `window_time_gap_median_seconds`, `window_time_gap_p90_seconds`, `window_time_gap_p75_seconds`, `window_time_gap_p25_seconds`, `window_time_gap_iqr_seconds`, `window_time_gap_p95_seconds`, `window_time_gap_p99_seconds`, `window_time_gap_total_seconds`, `window_time_gap_avg_seconds`, `window_time_gap_stddev_seconds`, `window_time_gap_cv_ratio`, `window_time_gap_mad_seconds`, `window_time_gap_mad_ratio`, `window_time_gap_outlier_count`, `window_time_gap_outlier_lower_seconds`, `window_time_gap_outlier_upper_seconds`, `window_time_gap_outlier_fence_span_seconds`, `window_time_gap_outlier_fence_midpoint_seconds`, `window_time_gap_outlier_effective_lower_seconds`, `window_time_gap_outlier_effective_span_seconds`, `window_time_gap_outlier_effective_midpoint_seconds`, `window_time_gap_outlier_effective_coverage_ratio`, `window_time_gap_outlier_lower_truncation_seconds`, `window_time_gap_outlier_lower_truncation_ratio`, `window_time_gap_outlier_lower_clipped`, `window_time_gap_outlier_lower_clipped_ratio`, `window_time_gap_outlier_lower_unclipped`, `window_time_gap_outlier_lower_unclipped_ratio`, `window_time_gap_outlier_lower_clip_state`, `window_time_gap_outlier_lower_clip_state_code`, `window_time_gap_outlier_ratio`, `window_time_gap_inlier_count`, `window_time_gap_inlier_ratio`, `window_time_gap_inlier_minus_outlier_count`, `window_time_gap_outlier_minus_inlier_count`, `window_time_gap_outlier_inlier_count_ratio`, `window_time_gap_inlier_outlier_count_ratio`, `window_time_gap_outlier_inlier_ratio_delta`, `window_time_gap_outlier_inlier_ratio_delta_basis_points`, `window_time_gap_outlier_inlier_ratio_delta_basis_points_abs`, `window_time_gap_outlier_inlier_ratio_delta_per_mille`, `window_time_gap_outlier_inlier_ratio_delta_percentage_points`, `window_time_gap_outlier_inlier_ratio_delta_percentage_points_abs`, `window_time_gap_outlier_inlier_ratio_delta_abs`, `window_time_gap_outlier_inlier_ratio_delta_sign`, `window_time_gap_outlier_inlier_ratio_delta_positive`, `window_time_gap_outlier_inlier_ratio_delta_nonpositive`, `window_time_gap_outlier_inlier_ratio_delta_negative`, `window_time_gap_outlier_inlier_ratio_delta_nonnegative`, `window_time_gap_outlier_inlier_ratio_delta_zero`, `window_time_gap_outlier_inlier_ratio_delta_nonzero`, `window_time_gap_outlier_inlier_ratio_delta_state`, `window_time_gap_outlier_inlier_ratio_delta_direction`, `window_time_gap_outlier_inlier_ratio_delta_direction_code`, `snapshot_at`, `order` (`id_desc`)
    ...
    - `window_time_parseable` = whether all current page `at` timestamps are parseable; `null` when page is empty
    - `window_time_unparseable_count` = number of current page records whose `at` is missing or not parseable; `null` when page is empty
    - `window_time_parseable_count` = number of current page records whose `at` is parseable; `null` when page is empty
    - `window_time_missing_at_count` = number of current page records whose `at` is missing; `null` when page is empty
    - `window_time_invalid_at_count` = number of current page records whose `at` is present but not parseable; `null` when page is empty
    - `window_time_missing_at_ratio` = `window_time_missing_at_count/returned_items` (6-decimal precision) when page is not empty; otherwise `null`
    - `window_time_invalid_at_ratio` = `window_time_invalid_at_count/returned_items` (6-decimal precision) when page is not empty; otherwise `null`
    - `window_time_unparseable_ratio` = `window_time_unparseable_count/returned_items` (6-decimal precision) when page is not empty; otherwise `null`
    - `window_time_parseable_ratio` = `(returned_items-window_time_unparseable_count)/returned_items` (6-decimal precision) when page is not empty; otherwise `null`
    ...
    - `window_time_gap_range_seconds` = `window_time_gap_max_seconds-window_time_gap_min_seconds` (6-decimal precision) when timestamps are parseable; otherwise `null`
    ...
    - `window_time_gap_nonzero_ratio` = `window_time_gap_nonzero_count/window_time_gap_count` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_median_seconds` = median absolute adjacent `at` delta in seconds (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_p90_seconds` = p90 absolute adjacent `at` delta in seconds (6-decimal precision, nearest-rank) when timestamps are parseable; otherwise `null`
    - `window_time_gap_p75_seconds` = p75 absolute adjacent `at` delta in seconds (6-decimal precision, nearest-rank) when timestamps are parseable; otherwise `null`
    - `window_time_gap_p25_seconds` = p25 absolute adjacent `at` delta in seconds (6-decimal precision, nearest-rank) when timestamps are parseable; otherwise `null`
    - `window_time_gap_iqr_seconds` = interquartile range (`p75-p25`) of absolute adjacent `at` deltas in seconds (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_p95_seconds` = p95 absolute adjacent `at` delta in seconds (6-decimal precision, nearest-rank) when timestamps are parseable; otherwise `null`
    - `window_time_gap_p99_seconds` = p99 absolute adjacent `at` delta in seconds (6-decimal precision, nearest-rank) when timestamps are parseable; otherwise `null`
    ...
    - `window_time_gap_stddev_seconds` = population standard deviation of absolute adjacent `at` deltas in seconds (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_cv_ratio` = coefficient of variation (`window_time_gap_stddev_seconds/window_time_gap_avg_seconds`) with 6-decimal precision when timestamps are parseable; returns `0.0` when average gap is zero; otherwise `null`
    - `window_time_gap_mad_seconds` = median absolute deviation of adjacent `at` deltas around `window_time_gap_median_seconds` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_mad_ratio` = `window_time_gap_mad_seconds/window_time_gap_median_seconds` (6-decimal precision) when timestamps are parseable; returns `0.0` when median gap is zero; otherwise `null`
    - `window_time_gap_outlier_count` = number of adjacent `at` deltas outside Tukey fences `[p25-1.5*iqr, p75+1.5*iqr]` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_lower_seconds` = Tukey lower fence (`p25 - 1.5*iqr`) in seconds (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_upper_seconds` = Tukey upper fence (`p75 + 1.5*iqr`) in seconds (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_fence_span_seconds` = Tukey fence span (`window_time_gap_outlier_upper_seconds-window_time_gap_outlier_lower_seconds`) in seconds (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_fence_midpoint_seconds` = Tukey fence midpoint (`(window_time_gap_outlier_upper_seconds+window_time_gap_outlier_lower_seconds)/2`) in seconds (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_effective_lower_seconds` = effective lower fence (`max(0, window_time_gap_outlier_lower_seconds)`) in seconds (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_effective_span_seconds` = effective fence span (`window_time_gap_outlier_upper_seconds-window_time_gap_outlier_effective_lower_seconds`) in seconds (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_effective_midpoint_seconds` = effective fence midpoint (`(window_time_gap_outlier_upper_seconds+window_time_gap_outlier_effective_lower_seconds)/2`) in seconds (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_effective_coverage_ratio` = effective coverage ratio (`window_time_gap_outlier_effective_span_seconds/window_time_gap_outlier_fence_span_seconds`) with 6-decimal precision when timestamps are parseable; returns `0.0` when fence span is zero; otherwise `null`
    - `window_time_gap_outlier_lower_truncation_seconds` = lower-bound truncation seconds (`max(0, -window_time_gap_outlier_lower_seconds)`) with 6-decimal precision when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_lower_truncation_ratio` = lower truncation ratio (`window_time_gap_outlier_lower_truncation_seconds/window_time_gap_outlier_fence_span_seconds`) with 6-decimal precision when timestamps are parseable; returns `0.0` when fence span is zero; otherwise `null`
    - `window_time_gap_outlier_lower_clipped` = whether Tukey lower fence is below zero (`window_time_gap_outlier_lower_seconds < 0`) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_lower_clipped_ratio` = `1.0` when `window_time_gap_outlier_lower_clipped=true` else `0.0` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_lower_unclipped` = whether Tukey lower fence is non-negative (`window_time_gap_outlier_lower_seconds >= 0`) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_lower_unclipped_ratio` = `1.0` when `window_time_gap_outlier_lower_clipped=false` else `0.0` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_lower_clip_state` = `"clipped"` when `window_time_gap_outlier_lower_clipped=true`, `"unclipped"` when `window_time_gap_outlier_lower_clipped=false` for parseable timestamps; otherwise `null`
    - `window_time_gap_outlier_lower_clip_state_code` = `1` when `window_time_gap_outlier_lower_clip_state="clipped"`, `0` when `window_time_gap_outlier_lower_clip_state="unclipped"` for parseable timestamps; otherwise `null`
    - `window_time_gap_outlier_ratio` = `window_time_gap_outlier_count/window_time_gap_count` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_inlier_count` = number of adjacent `at` deltas inside Tukey fences (`window_time_gap_count-window_time_gap_outlier_count`) when timestamps are parseable; otherwise `null`
    - `window_time_gap_inlier_ratio` = `window_time_gap_inlier_count/window_time_gap_count` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_inlier_minus_outlier_count` = `window_time_gap_inlier_count-window_time_gap_outlier_count` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_minus_inlier_count` = `window_time_gap_outlier_count-window_time_gap_inlier_count` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_count_ratio` = `window_time_gap_outlier_count/max(1, window_time_gap_inlier_count)` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_inlier_outlier_count_ratio` = `window_time_gap_inlier_count/max(1, window_time_gap_outlier_count)` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta` = `window_time_gap_outlier_ratio-window_time_gap_inlier_ratio` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_basis_points` = `round(window_time_gap_outlier_inlier_ratio_delta * 10000)` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_basis_points_abs` = `abs(window_time_gap_outlier_inlier_ratio_delta_basis_points)` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_per_mille` = `window_time_gap_outlier_inlier_ratio_delta * 1000` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_percentage_points` = `window_time_gap_outlier_inlier_ratio_delta * 100` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_percentage_points_abs` = `abs(window_time_gap_outlier_inlier_ratio_delta_percentage_points)` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_abs` = `abs(window_time_gap_outlier_ratio-window_time_gap_inlier_ratio)` (6-decimal precision) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_sign` = sign of `window_time_gap_outlier_inlier_ratio_delta` (`1` when positive, `-1` when negative, `0` when zero) when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_positive` = whether `window_time_gap_outlier_inlier_ratio_delta > 0` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_nonpositive` = whether `window_time_gap_outlier_inlier_ratio_delta <= 0` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_negative` = whether `window_time_gap_outlier_inlier_ratio_delta < 0` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_nonnegative` = whether `window_time_gap_outlier_inlier_ratio_delta >= 0` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_zero` = whether `window_time_gap_outlier_inlier_ratio_delta == 0` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_nonzero` = whether `window_time_gap_outlier_inlier_ratio_delta != 0` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_state` = `"positive"`, `"zero"`, or `"negative"` based on `window_time_gap_outlier_inlier_ratio_delta` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_direction` = `"outlier_heavier"`, `"balanced"`, or `"inlier_heavier"` based on `window_time_gap_outlier_inlier_ratio_delta` when timestamps are parseable; otherwise `null`
    - `window_time_gap_outlier_inlier_ratio_delta_direction_code` = `1`, `0`, or `-1` using the same mapping as `window_time_gap_outlier_inlier_ratio_delta_direction` when timestamps are parseable; otherwise `null`
